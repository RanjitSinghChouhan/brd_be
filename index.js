...
var accessLogStream = rfs.createStream(`${moment(`${new Date().toISOString()}`).format('DD-MM-YYYY')}/access.log`, {
...
app.use(morgan((function (tokens, req, res) {
  ...
  if (tokens.status(req, res) == 500 || tokens.status(req, res) == 400 || tokens.status(req, res) == 401 || tokens.status(req, res) == 403 || tokens.status(req, res) == 404) {
    info = 'ERROR';
  }

  let str = `{"byrds","IND","${req.socket.remoteAddress}","${appType}","${tokens.method(req, res)}","${tokens.url(req, res)}", ${tokens.status(req, res)},${tokens['response-time'](req, res)} ms}`
  return [
    tokens.date(req, res),
    info,
    str,
